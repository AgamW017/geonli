...
function ChatPage({ uploadedImage: initialImage, initialPrompt, sessionData, onNewChat, onLoadSession }) {
  ...
  useEffect(() => {
    const loadSessionImage = async () => {
      if (sessionData?.sessionId && !uploadedImage) {
    ...
    loadSessionImage();
  }, [sessionData?.sessionId, uploadedImage]);
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-full">
        <Topbar />
        <main className="flex-1 flex h-[calc(100%-4rem)] overflow-hidden">
          {imageLoading ? (
